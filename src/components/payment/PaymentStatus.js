...
import { CheckCircle2, AlertCircle, RefreshCcw, Home, Receipt } from 'lucide-react-native';
      ...
      <View style={styles.overlay}>
        <View style={[styles.card, { width: isLandscape ? 450 : '90%' }]}>
          <View style={[styles.iconCircle, { backgroundColor: current.bg }]}>
            <Icon size={60} color={current.color} />
          </View>
          ...
          <View style={styles.detailBox}>
            <View style={[styles.smallIcon, { backgroundColor: current.bg }]}>
                {status === 'success' ? <Receipt size={24} color={current.color} /> : <AlertCircle size={24} color={current.color} />}
            </View>
            <View>
          ...
          {(status === 'error' || status === 'awaiting') && (
            <TouchableOpacity style={styles.cancelBtn} onPress={onCancel}>
                <Home size={20} color="#94a3b8" style={{marginRight: 10}} />
